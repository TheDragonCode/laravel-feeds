...
use Workbench\App\Boost\GuidelineComposer;
...
#[AsCommand('boost:guidelines', 'Install guidelines from Laravel Boost')]
class InstallBoostCommand extends Command
{
    use Colors;
    ...
    {
        $this->systemInstalledCodeEnvironments  = ['phpstorm'];
        $this->projectInstalledCodeEnvironments = ['phpstorm'];
    }
    ...
    private function performInstallation(): void
    {
        ...
        if (
            ($this->shouldInstallMcp() || $this->shouldInstallHerdMcp())
            && $this->selectedTargetMcpClient->isNotEmpty()
        ) {
    ...
    private function selectCodeEnvironments(string $contractClass, string $label): Collection
    {
        ...
        $detectedClasses   = [];
        $installedEnvNames = array_unique(
            array_merge(
        ...
        $selectedClasses = collect(
            multiselect(
                ...
                hint    : empty($detectedClasses)
                    ? ''
                    : sprintf(
                        'Auto-detected %s for you',
        ...
        return $selectedClasses->map(
            fn ($className) => $availableEnvironments->first(fn ($env) => get_class($env) === $className)
        );
    }
    ...
    private function installGuidelines(): void
    {
        ...
        if (count($failed) > 0) {
            $this->error(
                sprintf(
                    '✗ Failed to install guidelines to %d agent%s:',
                    count($failed),
                    count($failed) === 1 ? '' : 's'
                )
            );
