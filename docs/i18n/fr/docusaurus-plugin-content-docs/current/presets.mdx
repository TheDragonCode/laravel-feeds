---
title: "Préréglages"
description: "Générer rapidement des flux avec les préréglages intégrés (Instagram, Yandex, Sitemap, RSS)"
type: reference
status: stable
since: "1.0"
...
[← Précédent](./location.mdx) · [Retour au README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Suivant →](./events.mdx)

# Préréglages

Vous pouvez créer rapidement des flux à l’aide des préréglages prêts à l’emploi :
...
Créez une classe de flux manuellement ou avec la commande `php artisan make:feed <name>`, puis étendez le préréglage requis. Exemple :
...
Implémentez ensuite les deux méthodes requises, `builder` et `item`. Exemple :
...
Techniquement, les classes de préréglage sont identiques à la classe de base `Feed`, mais elles intègrent la logique propre au service ciblé, notamment la construction des blocs d’informations et des éléments.
