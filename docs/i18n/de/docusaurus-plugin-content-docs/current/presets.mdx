...
description: "Feeds schnell mit integrierten Voreinstellungen erzeugen (Instagram, Yandex, Sitemap, RSS)"
type: reference
status: stable
since: "1.0"
...
[← Zurück](./location.mdx) · [Zurück zur README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Weiter →](./events.mdx)

# Voreinstellungen

Mit den sofort einsatzbereiten Voreinstellungen können Sie Feeds schnell erstellen:
...
Erstellen Sie eine Feed-Klasse manuell oder mit dem Konsolenbefehl `php artisan make:feed <name>` und erweitern Sie die benötigte Voreinstellung. Beispiel:
...
Implementieren Sie anschließend die beiden erforderlichen Methoden `builder` und `item`. Beispiel:
...
Technisch entsprechen Voreinstellungsklassen der Basisklasse `Feed`. Zusätzlich enthalten sie jedoch die dienstspezifische Logik ihres jeweiligen Ziels, einschließlich des Aufbaus von Informationsblöcken und Elementen.
