---
title: "Предустановки"
description: "Быстрая генерация фидов со встроенными предустановками Instagram, Yandex, Sitemap и RSS"
type: reference
status: stable
since: "1.0"
...
[← Предыдущая](./location.mdx) · [Назад к README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Следующая →](./events.mdx)

# Предустановки

Фиды можно быстро создавать с помощью готовых предустановок:
...
Создайте класс фида вручную или консольной командой `php artisan make:feed <name>` и расширьте нужную предустановку. Например:
...
Затем реализуйте два обязательных метода: `builder` и `item`. Например:
...
Технически классы предустановок такие же, как базовый класс `Feed`, но они содержат логику целевого сервиса, включая построение информационных блоков и элементов.
...
- [Рецепт для Instagram](./receipt-instagram.mdx)
