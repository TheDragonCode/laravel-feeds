---
title: "Predefinições"
description: "Gere feeds rapidamente com predefinições incluídas (Instagram, Yandex, Sitemap, RSS)"
type: reference
status: stable
since: "1.0"
...
[← Anterior](./location.mdx) · [Voltar ao README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Próximo →](./events.mdx)

# Predefinições

Você pode criar feeds rapidamente com as predefinições incluídas:
...
Crie uma classe de feed manualmente ou com o comando de console `php artisan make:feed <name>` e estenda a predefinição necessária. Por exemplo:
...
Depois, implemente os dois métodos obrigatórios: `builder` e `item`. Por exemplo:
...
Tecnicamente, as classes de predefinição são iguais à classe base `Feed`, mas incluem a lógica específica do serviço correspondente, inclusive a forma de montar os blocos de informações e os itens.
