...
description: "Хуткая генерацыя стужак з убудаванымі перадусталёўкамі Instagram, Yandex, Sitemap і RSS"
type: reference
status: stable
since: "1.0"
...
[← Папярэдняя](./location.mdx) · [Назад да README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Наступная →](./events.mdx)

# Перадусталёўкі

Стужкі можна хутка ствараць з дапамогай гатовых перадусталёвак:
...
Стварыце клас стужкі ўручную або кансольнай камандай `php artisan make:feed <name>` і пашырце патрэбную перадусталёўку. Напрыклад:
...
Затым рэалізуйце два абавязковыя метады: `builder` і `item`. Напрыклад:
...
Тэхнічна класы перадусталёвак такія ж, як базавы клас `Feed`, але яны змяшчаюць логіку мэтавага сэрвісу, у тым ліку пабудову інфармацыйных блокаў і элементаў.

## Стандартныя значэнні {/* #defaults */}
