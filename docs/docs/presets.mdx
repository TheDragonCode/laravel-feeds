---
title: "Presets"
description: "Generate feeds quickly with built-in presets (Instagram, Yandex, Sitemap, RSS)"
type: reference
status: stable
since: "1.0"
...
[← Previous](./location.mdx) · [Back to README](https://github.com/TheDragonCode/laravel-feeds#readme) · [Next →](./events.mdx)
...
Create a feed class manually or via the `php artisan make:feed <name>` console command and extend the preset you need. For example:
...
Then implement the two required methods — `builder` and `item`. For example:
