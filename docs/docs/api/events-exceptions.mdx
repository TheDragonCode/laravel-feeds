...
Target selection uses SPL exceptions. `feed:generate --target` throws `InvalidArgumentException` when the feed ID or key is missing, the key is empty, or the feed does not implement `HasFeedTargets`. It throws `UnexpectedValueException` when `findTarget()` returns `null`.
